...
Pipeline (exact match to sam-img2img__1_.ipynb / evaluation_pipeline.ipynb):
  ...
  Step 3 – ControlNet img2img
    ...
    strength=0.40  |  guidance=7.5  |  controlnet_scale=0.6–0.7  |  steps=25
...
Environment variables (all optional — sensible defaults shown):
  ...
  CONTROLNET_CONDITIONING_SCALE  "0.6"   (kept inside the requested 0.6–0.7 range)
...
IMG2IMG_STRENGTH:      float = float(os.environ.get("IMG2IMG_STRENGTH", "0.40"))
IMAGE_SIZE:            int   = 512
...
GENERATION_NEGATIVE_PROMPT = (
    ...
    "text, logo, watermark, clutter, multiple garments"
)
...
def _parse_color_hints(
    ...
    Painted = pixel is not exactly pure white (255,255,255), the canvas
    background colour. Exact RGB pixel match only — no HSV/saturation
    threshold is used.
    """
    img = (
        color_hints_img
    ...
    painted = ~np.all(img_uint8 == 255, axis=-1)
...
def _assign_colors(
    ...
    for m in masks:
        ...
        if region_hint_mask.any():
            # Median in LAB space — more perceptually accurate than RGB median
            hints_rgb        = color_map[region_hint_mask]                          # (N,3) float32 [0,1]
            hints_lab        = rgb2lab(hints_rgb.reshape(1, -1, 3)).reshape(-1, 3)  # (N,3) LAB
            median_lab       = np.median(hints_lab, axis=0)                         # (3,) LAB
            region_color     = lab2rgb(median_lab.reshape(1, 1, 3)).reshape(3,)     # (3,) RGB [0,1]
    ...
    if has_hints and not filled_mask.any() and outer_mask.any():
        hints_rgb               = color_map[hint_mask[:, :, 0] > 0.05]
...
def _apply_color_pipeline(
    ...
    logger.info("  LAB blend only (σ=2.0) — single clean blend pass …")
    blended = _lab_blend(cn_float, colored_np, filled_mask, blend_sigma=2.0)
...
def generate_image_controlnet(
    ...
    img2img_strength float [0,1].  0.40 recommended.
